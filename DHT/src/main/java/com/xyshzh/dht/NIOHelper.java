...
public class NIOHelper {
      ...
      worker.submit(new Callable<Boolean>() {
        ...
        public Boolean call() throws Exception {
          try {
            ...
            // 注册该通道到选择器, 并关注读事件, 这个注册可能会阻塞, 因为选择器已经开始阻塞在select方法上,
            // 但是选择器每隔10毫秒会中断一次, 所以这个阻塞时间很短
            SelectionKey key = channel.register(SELECTOR, SelectionKey.OP_READ);
            key.attach(new Attachment(id));

            // 添加节点id到选择键的映射关系, 对id2SelectionKey的读写操作都必须放到worker这个线程中进行,
            // 这样就可以消除线程同步的工作
  ...
  class Attachment {
    ...
    private final Queue<WriteData> writeDataQueue = new LinkedList<>(); // 用来存放附件所在通道需要写出的数据对象,
                                                                        // 对于这个队列的读取都是单线程的,
                                                                        // 所以不用做同步
