...
public class YARNRunner implements ClientProtocol {
  ...
  /**
   * Similar to {@link YARNRunner#YARNRunner(Configuration, ResourceMgrDelegate)} but allowing
   * injecting {@link ClientCache}. Enable mocking and testing.
   * 
   * @param conf
  ...
  void addHistoryToken(Credentials ts) throws IOException, InterruptedException {
    ...
    if (UserGroupInformation.isSecurityEnabled() && (hsProxy != null)) {
      /*
       * note that get delegation token was called. Again this is hack for oozie to make sure we add
       * history server delegation tokens to the credentials
       */
  ...
  public JobStatus submitJob(JobID jobId, String jobSubmitDir, Credentials ts)
    ...
    ApplicationSubmissionContext appContext = createApplicationSubmissionContext(conf, jobSubmitDir,
        ts);
  ...
  private LocalResource createApplicationResource(FileContext fs, Path p, LocalResourceType type)
    ...
    rsrc.setResource(URL.fromPath(fs.getDefaultFileSystem().resolvePath(rsrcStat.getPath())));
  ...
  public ApplicationSubmissionContext createApplicationSubmissionContext(Configuration jobConf,
    ...
    capability
        .setMemorySize(conf.getInt(MRJobConfig.MR_AM_VMEM_MB, MRJobConfig.DEFAULT_MR_AM_VMEM_MB));
    ...
    if (jobConf.get(MRJobConfig.JAR) != null) {
      ...
      String pattern = conf
          .getPattern(JobContext.JAR_UNPACK_PATTERN, JobConf.UNPACK_JAR_PATTERN_DEFAULT).pattern();
    ...
    for (String s : new String[] { MRJobConfig.JOB_SPLIT, MRJobConfig.JOB_SPLIT_METAINFO }) {
      localResources.put(MRJobConfig.JOB_SUBMIT_DIR + "/" + s, createApplicationResource(
          defaultFileContext, new Path(jobSubmitDir, s), LocalResourceType.FILE));
    }
